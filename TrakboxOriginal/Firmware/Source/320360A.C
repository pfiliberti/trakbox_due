...
/*****************************************************************/
/* 80C320 porting 19200bps support		     13 Mar 94   */
/* Memory Uplink channel support		     24 Apr 94   */
/* ADC clock devider changed to 1/8		     29 Apr 94   */
/* mic click pulse width and interval value          15 May 94   */
/* used [data] memory for speed up and code size save 15 May 94  */
/* Flip mode installed			v3.30a	     30  May 94  */
/* ADC satellite selection SW moved to s/w SW        30  May 94  */
/* Multi track change over improve                   20  Jun 94  */
/* Flip OFF in parking action           v3.30b       25  Jun 94  */
/* Changeover improved in multi tracking             25  Jun 94  */
/* Bug fix in Parking freq set          v3.30d                   */
/* REG_MAX = 60                         v3.30e       21  Aug 94  */
/* Function to assign all sats in RS buffer v3.30e   21  Aug 94  */
/* Host mode call from real time tracking            22  Aug 94  */
/* bug fix at source USN adc_dat -> adc_dat   v3.30f 31  Aug 94  */
...
/* Bug fix in North start flip routine          v3.30h 09 Oct  94  */
/* Bug fix in leep year decision                v3.30i 04 Jan  94  */
/* ICOM id display optimize, return value for getline()  3.30k  Mar. 07 */
/* Minor mods to ICOM CAT for IC-821 support    v3.30m   Aug.08 1996 */
/*            VFO-A/B selection is ignored */
/* The year of 2000 support  in schedule, G2 selection  v3.40a Dec.01 1996*/
/* Bug fix in Flip mode decision (|= (& ~xx))		v3.40a Jan.09 1997 */
/* Introduce configurable err_timeout value		v3.40a Jan.17 1997 */
/* Default kepele of 1997				v3.40a Jan 18 1997 */
/* Bug fix in ICOM 970/821 CAT				v3.40c Apr.03 1997 */
/* FT-736 dummy freq. 1.2Ghz -> 51.0Mhz */
/* Fix in 1200Mhz capable in YAESU CAT                  v3.40c Apr 08 1997 */
/* Fix 1200Mhz capable in host mode freq. command	v3.40c Apr 08 1997 */
/* Fix 2400MHz(USN long value) capable in freq. set	v3.40d May 10 1997 */
/* Freq offset capable in host mode			v3.40d May 11 1997 */
/* Code size optimize					v3.40e June 15 1997 */
/* v3.40e release					       Sep. 10 1997 */
/* lpt baud rate configurable */
/* 320340fx    YAESU FT-847 temp */
/* icom_wait(), so 970 and 821 are unified.		v3.50  Oct.01 1998 */
/* v3.50a  test release      */
/* yaesu_cmd(,) introduce. 				v3.50a Oct.18 1998 */
/* parking function even in main menu 			v3.50a Oct.31 1998 */
/* official release					v3.50a Dec.03 1998 */
/* bugfix in months(),get_epday() endless loop in for loop */
/* RTC init to "00/01/01 00:00:00"			v3.50b Dec.15 1998 */
/* lcd_cmd(), lcd_data() moved to C */
/*	i_wait introduced */
/*	ICOM_SEPARATE introduced		Jan.17 1999 	*/
/*	removed 0x07 command in ICOM_SEPARATE trk350cy	Jan.20 1999 */
/* enable i_wait configulation from STATION.DAT */
/* default Kepele of 1999 */
/*	bug fix in "ir" option read in scanf()  Feb.07 1999 */
/*   bug fix in K0 input from keybourd long value 	Apr.20 99 */
/* start v350d		jan.03 2000   */
/*		minor change in kepele file reading line[2] == 'A' */

/* v3.60a	Y2020 support rename  Sep.22 2013 */
...
#include <ds80c320.h>              /* define 8051 registors */
...
#define    AZ_MINI     11       /* default mini. adc value for AZ */
#define    AZ_MAX     874      /* default Max. adc value for AZ */
#define    EL_MINI     2       /* default mini. adc valu  for EL */
#define    EL_MAX     940      /* default Max. adc valu  for EL */

#define    ERR_TIMEOUT	120	/* rotor errot timeout time */
#define	   ERR_RECOVER_TM 61	/* Auto recover from err mode */

#define    TRACK_START  -2.0   /* Tracking start EL value  */
#define    EL_BLINK     -3.0   /* led2 brinking deg.            */
#define	   P16_ON	-5.0   /* p16_on_el value */
#define	   P16_OFF	-6.0   /* p16_off_el value */
...
#define		MIC_CLICK	0x0001	/* mic_click option ADC value set ? */
#define		HOST_FCONV	0x0002  /* freq offset in host mode */
...
#define		YAESU_736	0x01
#define		YAESU_847	0x02
#define		YAESU_OTHER	0x04

#define		ICOM_SEPARATE	0x10
...
char   	obj_sat;        /* Tracking object satellite (-1, 0-14) */

USN  long   mem_DX_az,mem_DX_el;
USN  int    err_timeout,err_recover_tm;

static code USN char *greeting[]={
	"\r\n\r\n\r\n\t****************************************\r\n",
	"\t*       TrakBox  80C320  v3.60a        *\r\n",
...
float  LOCAL_INF[5];	/* 0:local_lat, 1:local_lon, 2:local_lin_c, 3:local_high */

USN char   pulse_w,pulse_i;    /* mic click pulse width and interval */
...
static USN int lpt_baud_tab[5]=
	{0xFA00,0xFD00,0xFE80,0xFF40,0xFFA0};   /* 1200,2400,4800,9600,19200 */

static USN char baud_tab[6]=
	{0x40,0xD0,0xE8,0xF4,0xFA,0xFD};	/* 300-19200 */

USN char baud_rate,lpt_baud_rate;		/* 0,1,2,3,4,5 */
...
#ifdef NOTDEF
float G2_tbl[20]={   /* Greenwitch Sideral Time table  1996-12-01 */
	0.275442,	/* 1991 */
	0.274779,
	0.276854,
	0.276191,
...
USN short   Y02[SATMAX];         /* epoch Year = 97, 98, 99,, */
float   T02[SATMAX];
float   M02[SATMAX];
float   N02[SATMAX];
float   I02[SATMAX];
...
USN char UPLINK_CH[SATMAX];	/* Uplink channel */
...
char	sat_list[REG_MAX+1];	/* Control SAT LIST  -1 -- REG_MAX  */
...
USN int      sat_mx;         /* RAM stored max number */
...
void   spout2(UCHAR);		    /* Serial printer out-2 */
...
void   main(void);
void   print_lcd(char *) small;           /* This is  C code  */
void   months(void) small;
void   send(char *) small;
USN short  day_of_year(USN short,USN char,USN char);
void   sel_sat(char);
void   set_def(void);
...
void   sendp2(char *) small;                  /* out to Alternate LP port */
void   read_kep(void);
...
USN char   read_mode(void);        /* binary SW and R-ADC read & mode   */
void   disp_adc(void);
void   mic_step_con(void);
void   host_mode(void);	       /* host mode  92-09-28 */
void   rotor_error();
void   el_az_deg_set();
void   flip_check(char);	/* 94-05-25 */
void	wait(USN int);			/* 96-08-05 */
void	reset_timeout();
...
char	   num;
	   ...
	   SW2 &= ~FLIP_ON;		/* flip mode off */

   	   wait(30000);		/* wait for slow LCD initialization */
           print_lcd(greeting[4]);      /* Title display 1992/09/25 */
           lcd_cmd(0xc0);		/* LCD line-2 */
	   print_lcd(greeting[6]);	/* print copy right */
	   
	   reset_timeout();	/* reset rotor error timer, include month() */
	   if(T > 396) send("\n\r\x07Too old elements.\r\n");	   

           mode = read_mode();                  /* Read RUN MODE */

           if(mode){			/* standalone mode */
	       SW |= RUN_MODE;
               send("\n\r\x07");
               for(;;){
		       XBYTE[PPI+2] &= 0xf0;   /* Stop AZ/EL rotor */	       
		       mode = read_mode();
               	       obj_sat = mode - 1;
...
/*   CKCON |= 0x07;			80C320 peripheral speed */

   XBYTE[NEC_ADC+1] = 0x03;            /* ADC INIT 1/8 clock sampling Binary */
				       /* 1/4:0x02, 1/2:0x01 1/1:0x00 */
   ...
   if((XBYTE[PPI+1] & RSP1200B) == 0){
	 ...
	 TH1 = 0xFA;		/* default 9600bps */
					  ...
					  4:9600 bps 5:19200   */
   		   /* Serial baud rate 9600   256-sys_clock/(384* b. rate) */ 
...
void   set_first()
{
	   ...
	   lpt_baud_rate = 2;	/* 4800 bps   for YAESU/TS-790 */
	   ...
	   AZ_park = EL_park = 0.0;	/* Az/Elevation Parking Position */
	   ...
	   icom_t = i_tx_address = ICOM_T_AD;	/* ICOM TX destination address */ 
           ...
           yaesu_tx_cmd = 0x2E;	/* FT-874 = 21,11 */
	   yaesu_rx_cmd = 0x1E;

	   i_wait = 50;		/* 50msec  */

	   pulse_w = 1;			/* mic click pulse width */
	   pulse_i = 2;
	   step_size    = 100;		/*  step freq. of mic click */
           pll_lock     = ADC_CEN;      /*  TS790 default  */
	   pll_reff     = 560;          /*  1KHz off point */
	   err_recover_tm = ERR_RECOVER_TM;	/*  Error mode timeout time */
	   err_timeout = ERR_TIMEOUT;

#ifdef NOTDEF
           SW &= ~MIC_CLICK;            /* Mic click not selected */
           SW &= ~HOST_FCONV;		/* freq offset in host mode */
	   ...
	   SW &= ~RUN_MODE;		/* terminal mode */
	   SW &= ~MULTI_MODE;		/* multi-tracking mode not selected  */
	   SW &= ~BREAK_M;	/* reset break from s-track routine in m-track flag */
	   SW |= PRIORITY_TRK; /* reset priority tracking flag */
	   SW |= DOPPL_UPL;	/* uplink Doppler correction ON */
	   SW |= DOPPL_DWN;	/* downlink Doppler correction ON */
	   ...
	   SW2 &= ~P16_ON_FLAG;  /* P16 On flag */	   
	   SW2 &= ~PARK_TRX_MODE; /* TRX mode/freq. set */
	   SW2 |= ERR_MODE_OFF;   /* default OFF */ 
           SW2 &= ~FLIP_MODE;
	   SW2 &= ~FLIP_ON;
#endif
	   SW3 |= YAESU_736; SW3 &= ~YAESU_847;
...
void 	clr_buff() small   /* clear the data area */
{
data  USN char  i;
	   for(i=0;i<SATMAX;i++){
		 MDU[i] = MDD[i] = '\0';	   
		 Y02[i]= M02[i]= N12[i]= UPLINK_CH[i] = 
		 N02[i]= I02[i]= E02[i]= W02[i]= O02[i]= 0;
		 K02[i]= T02[i]= 1;
		 F01[i]= F02[i][0]=F02[i][1]=F02[i][2]=F02[i][3]= 0;
		 F0CONV[i] = F1CONV[i]= CORRECT_MIC[i] = 0;
...
USN char  read_mode()         /* Binary SW read & ADC READ_MODE */
{
...
void   sendp2(s) small                  /* out to Alternate LP port */
char   *s;
{
data  USN char  ch;
...
USN     char   *Mdmsg[] = {	/*  TX/RX mode  msg */
           ...
           if(getline(tmp)) strcpy(O_name[s_num],tmp);
...
void radio_p_conf()	/* Radio CAT port configuration */
{
	...
	UINT	temp;
	       ...
	       send("\r\n\n\tCAT Rig KENWOOD,YAESU,ICOM [K/Y/I] ?  ");
		 	...
		 	else if(( ch | 0x20 ) == 'n') SW &= ~YAESU_FMN;
		   } 			 		 
		   else if((ch | 0x20) == 'i'){
		         rig_maker = ICOM;
			 printf("\r\n\tEnter destination Address for RX (%BX) ",
                                        i_rx_address);
                         if(getline(tmp))
	                         sscanf(tmp,"%hx",&i_rx_address);
 
                         printf("\tEnter destination Address for TX (%BX) ",
                                        i_tx_address);
                         if(getline(tmp))
	                         sscanf(tmp,"%hx",&i_tx_address);
		...
		if(lpt_baudval == 1200) lpt_baud_rate = 0;
		else if(lpt_baudval == 2400) lpt_baud_rate = 1;
		else if(lpt_baudval == 4800) lpt_baud_rate = 2;
		else if(lpt_baudval == 9600) lpt_baud_rate = 3;
...
USN     char    lcd_text[50],mark = NON_ST;
	...
	for(i=0;i<SEL_MAX;i++) ctr_az[i] = ctr_el[i] = '\0';
  	...
  	SW2 |= PARK_TRX_FLAG;
		...
		if((sat_list[obj_sat] < 0) || (obj_sat >= m_track_num)){
	       		printf("\x07\r\n\tOut of number ! (1 - %bd)  #1 was selected for LCD display\r\n",m_track_num);		
			obj_sat = 0;
		}
        	sel1 = obj_sat;
	}
		...
		if(SW & RUN_MODE){  /* standalone mode */
			...
			if(mode >= m_track_num){			       
				lcd_act_sat = sat_list[0];
                        ...
                        if((XBYTE[PPI+1]^ 0xff) & ANT_CONT){  /* Test cont SW */
                                SW |= TRACK_ID;
                        }
                        else {
                                SW &= ~TRACK_ID;
...
void tracking2()		/* one satellite tracking */
{
   ...
   USN char    lcd_text[50],mark,no_mark = NON_ST,mark_ = TRACK_ST;
...
newsat_found:
	   ...
	   if(sat_list[obj_sat] < 0) continue;
   	   ...
   	   fd0_los = 0;			/* init fd0 and set step flag */
	   trk_frq_correct = 0;		/* manual correction value 0 */

	   if(rig_maker == YAESU){
	   	yaesu_cmd(0x00,0x00);	/* CAT ON */
		if(SW3 & YAESU_736){  /* YAESU FT-736  */
			spout(Y_WAIT,0xc9);spout(Y_WAIT,0x60);spout(Y_WAIT,0x00);   /* 92-10-17 */
        	       	spout(Y_WAIT,0x00);spout(Y_WAIT,yaesu_rx_cmd);	/* dummy 1296.0MHz */
			spout(Y_WAIT,0x05);spout(Y_WAIT,0x10);spout(Y_WAIT,0x00);   /* 97-04-02 */
               		spout(Y_WAIT,0x00);spout(Y_WAIT,yaesu_rx_cmd);	/* 51.0000MHz */
		}
		else if(SW3 & YAESU_847) yaesu_cmd(0x00,0x4E); /* sat ON */
		yaesu_cmd(0x00,0x80);
	   }
           trk_frq_d = RFD;		/* dummy */	   
           trk_frq_u = RFU;		/* dummy */
	   ...
	   if(trx_mode[1] == FM) trk_frq_band = (long)TRK_FRQ_FM; /* 200Hz step  */
           else trk_frq_band = (long)TRK_FRQ;      /* default 20Hz step */
...
/*******************************************************/
		   ...
		   if((SW & MULTI_MODE) == 0){  /* if not multi mode */
			...
			return;
                   ...
                   if((XBYTE[PPI+1]^ 0xff) & ANT_CONT){  /* ANT cont SW PB4 */
                           SW |= TRACK_ID;
                           mark = mark_;
                   }
                   else {  
	     ...
	     if (obj_sat_mem != obj_sat) goto newsat_found;
	     ...
	     if(E9 >= p16_on_el){
	     		...
	     		SW2 |= P16_ON_FLAG;
			if((SW2 & FLIP_MODE) && (SW & TRACK_ID) && /*94-05-25*/
			...
			SW2 |= PARK_TRX_FLAG;
		   	SW2 &= ~FLIP_ON;
			return;  /*  if multi mode, return to tracking1() */
	     }
...
char   sel;
{
        float   AZ_flip = A9;	/* mem present Az dimension */

        XBYTE[PPI+2] &= 0xf0;   /* Stop AZ/EL rotor */

        for(;;){
             if(N0 >= 10.0) T += 0.0006944444;  /* 1min. interval */     
             else T += 0.00694444;              /* 10 min. interval */

             main_loop(sel);        /*  Culculation sat   */ 
             lcd_cmd(0xc0);            /* set DD RAM to line-2 top */
             print_lcd("QRX Flip check..");;
/*
             printf("\t\r\nT = %f  AZ = %4.1f    EL = %4.1f",T,A9,E9);
*/           
             if((SW & ROTOR_ZERO) == 0){        /* South start */
                  if(((AZ_flip > 90) && (AZ_flip < 180.0) && (A9 >= 180.0))
                  || ((AZ_flip < 270) && (AZ_flip >= 180.0) && (A9 < 180.0))){
                                SW2 |= FLIP_ON;
                                break;
                  }
             } 
...
*/		
		...
		if((tm_sec == ts) && (tm_min - tm) == err_recover_tm){
	     ...
	     mem_DX_az = mem_DX_el = DX;  /* reset timeout timer mem. */
}
...
void   yaesu_cmd(UCHAR D_1,UCHAR P_1) small
{
                spout(Y_WAIT,D_1);
                spout(Y_WAIT,0x00);
                spout(Y_WAIT,0x00);
                spout(Y_WAIT,0x00);
                spout(Y_WAIT,P_1);   /* CAT off command   */
}

void icom_cat_header(ch)
USN	char	ch;	/* destination address */
{
                spout(0,0xfe);spout(0,0xfe);
                spout(0,ch);spout(0,0x02);
}
   ...
   USN long     fl;
		...
		if((s_txt[0] == '1') && (s_txt[1] =='2')){
			...
			chl = (s_txt[2]-0x30) & 0x0f;
                ...
                spout(Y_WAIT,yaesu_rx_cmd);                   /* FULLDUP RX  Freq  */
		yaesu_cmd(0x00,0x80);
        }
...
char	sel;
{
	USN char    D1,s_txt2[40],s_txt3[4];
		...
		if(!(SW3 & ICOM_SEPARATE)){
			...
			icom_cat_header(icom_t);
                        spout(0,0x01);   /* Set mode command */
			switch(trx_mode[0]){
				case LSB: spout(0,0x00);break;
				case USB: spout(0,0x01);break;
				case CW:  spout(0,0x03);break;
				case FM:  spout(0,0x05);break;
			}
			spout(0,0xfd);
			icom_wait(i_wait);
...
USN short day_of_year(year,month,day)
USN short year;
USN char month,day;
{
	USN int 	i,leep;
...
int	g2_sel;
      ...
      Y2 = Y02[sel]+1900;
      if(Y02[sel] < 90) Y2 += 100;
      g2_sel = Y2 - 1991;
      if((g2_sel<0) || (g2_sel>19)) g2_sel = 19;      
...
/*       G2 = G2_tbl[(Y02[sel]-90)%10];	 G2 selection */

       G2 = G2_tbl[g2_sel];	 /* G2 selection  1996-12-01 */
       S1 = sin(I0*P0);
       C1 = cos(I0*P0);
       ...
       Z9 = R9 *S9; /* sin(L8);*/
       X9 = R9 *C9*C8; /*cos(L8)*C8;*/
       Y9 = R9 *C9*S8;/* cos(L8)*S8;*/
...
USN     char    i;       
      ...
      F0CONV[1] = F1CONV[1] = 0;
      memcpy(O_name[1],"FO-29\0",6);

      Y02[2] = 13;
      T02[2] = 254.54205523;     /* Epoch Time */
      K02[2] = 77725;           /* Epoch rev. */
      M02[2] = 323.8264;        /* Mean Anomaly */
      N02[2] = 14.29120366;     /* Mean Motion */
      I02[2] = 31.4339;         /* Inclination */
      E02[2] = 0.0351235;       /* Eccentricity */
      W02[2] = 193.9814;        /* Arg. of Perigee */
      O02[2] = 298.6696;        /* R.A.A.N. */
      N12[2] = 4.94e-06;        /* Decay rate */
      F01[2] = 435.910;
      F02[2][0] = 145.870;
      F02[2][1] = 145.850;
      F02[2][2] = 145.890;
...
void disp_adc()		/* display PLL ADC value  */
{
	read_adc(PLL_ADC);
...
char	sel;
{
       if((SW & TRACK_ID) == NO){
		...
		return;
           ...
           if(EL_setv >= EL_BLINK){ /* AOS LED blinking routine */
               ...
               if(SW & MIC_CLICK){
	       		if((FREQ1 > 0.1) && (SW2 & FLAG_AOS_CAT)) rig_set_d(sel);
	   ...
	   if((SW & ANT_PARK) && (EL_setv < p16_on_el)){
			...
			rotor_con2();
#endif
			EL_setv = - 15.0; /* dummy for next rotor_con1 call */
			return;
	   }
	   else reset_timeout();
           XBYTE[PPI+2] &= 0xF0;     /* stop rotator */
		...
		if(SW2 & MIC_CAT_FLAG){		
	       		if((FREQ1 > 0.1) && (SW2 & FLAG_AOS_CAT)){
				rig_set_d(sel);
			}
			SW2 &= ~MIC_CAT_FLAG; 
		}
		fd0_los = RFD;	/* copy freq. for step control */
       		XBYTE[LED_PORT] |= AOS_LED;	/* AOS LED ON */
...
/**/
       ...
       else if((ch | 0x20) == 'n') SW &= ~ROTATE_L;

       send("\r\n\tSet rotor smoke protection ? [Y/N]  ");
       ch = getchar();
       if((ch | 0x20) == 'y'){
       		SW2 &= ~ERR_MODE_OFF;
		...
		if(cs != 0) err_timeout = css; 
       		...
       		printf("\r\n\tAuto recovery time from error mode [1-61 min] : %3d",err_recover_tm);
		css = (USN int)get_val("\t",&cs);
		if(cs != 0) err_recover_tm = css; 
       }
       else if((ch | 0x20) == 'n') SW2 |= ERR_MODE_OFF;
}       
...
USN short temp = SW;
       ...
       if (SW & ANT_PARK){
		...
		if(SW2 & PARK_TRX_MODE){/* send("\tTRX  parking      On\r\n");*/
       ...
       if(SW2 & ERR_MODE_OFF){
       		send(          "\tRotator smoke protection\tOff\r\n");
       }
       else{
		printf(        "\tTimeout value for rotator error: %3d sec.\r\n",err_timeout);
		if(err_recover_tm < 61){
       			printf("\tAuto recovery time from rotor error: %3d min.\r\n",err_recover_tm);
		}
       }
       ...
       switch(rig_maker){
			...
			break;
		case YAESU:
			if(SW3 & YAESU_736) send("  FT-736");
			else if(SW3 & YAESU_847) send("  FT-847");
...
short num;
{
   USN short   day_of_year,yx,y_dsp;
   USN char    m_dsp,d_dsp,h_dsp,mi_dsp,s_dsp,mox,dx,hx,mix,sx;
   USN char    ch,i,flg = NO, scr_flg = NO,temp[40];
   short	cs;
   float   minx = 1.0,secx = 0.0;
	   ...
	   minx *= 0.0006944443;              /*Day / sec */
           ...
           for(;;){
		       ...
		       scr_flg = NO;
                   ...
                   if(E9 > -1.0){
                       printf("%04d/%02bd/%02bd %02bd:%02bd:%02bd    %5.1f/%4.1f%9.1f%+9.0f\r\n",
		   ...
		   else T += 0.00069444;/*  step up + 1 min.  for speed up */
		   tmp_sat_el = E9;
...
USN	int	n,i;
       ...
       ts = (float)sx/86400.0;
#ifdef NOTDEF
       if(Y2 == (yx-1)){	      /* 1992/04/30 */
	        if((yx-1)%4 == 0) /* && (yx-1)%100 != 0 || (yx-1)%400 == 0) */ 
        	     DA += 366.0;	/* last year equal leep */
		else DA += 365.0;
       }
#endif
...
{
	float fx1;
USN	char   h_dsp,mi_dsp,s_dsp;

           fx1 = tos * 24.0;
           h_dsp =(USN char) fx1;
           fx1 =fx1 - (float)h_dsp;
           mi_dsp = (USN char)( fx1*60.0);
           fx1 = fx1 -(float)(mi_dsp/60.0);
           s_dsp = (USN char)(fx1*3600.0);
           *th = h_dsp;
           *tm = mi_dsp;
           *ts = s_dsp;
}
...
char disp_list_sat()     /*  display assigned satellite */
{
USN	char   ch;
	short   i,cs,st;

       send("\r\n");
       for(i=0;i<REG_MAX;i++){         /* 0 - REG_MAX */
...
{		          /* 92-04-14 */
USN   char    k,m=0,n,j,o,p,q,r;
USN   int     i;
USN   char    ch,tmp[100],tmp1[20],tmp2[40],tmp21[20],
...
{
   USN int     i,lpt_baudval=0,baudval=0,err_lavel=0;
   USN char    k,ch;
   USN char    tmp1[80],tmp5[20],tmp6[20],tmp7[20];
	...
	for(i=rs_buff_mark;;){  
		...
		tmp6[0] = tmp5[1]; tmp6[1] = '\0';	/* 94-05-15 */
		pulse_w = (short)atoi(tmp6)+1;
		tmp6[0] = tmp5[2]; tmp6[1] = '\0';
		pulse_w = (short)atoi(tmp6)+2;
...
{
USN	 char	ch,k,size,tmp[30],temp[80],temp1[30],temp2[30];
