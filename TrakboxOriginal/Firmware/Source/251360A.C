...
/* icom_wait(), so IC970 and 821 are unified   v3.50  01 Oct  98  */
/* bug fix in kepele append function */
/*******************************************************************/
/* NEW for 80C251 MPU,   double float calc. support  Oct. 18 */
/* Need START251.A51 for WSA=2 extend 2 wait states Dec 06 98 */
/*	UCONFIG0 = 0xBF, UCONFIG1 = 0xFF  */
/* Bug fix in Too old element display in RTC set mode */
/* set RTC 2000/01/01 00:00:00 in set_first() */
/* C251 difference
	sscanf(*s "%lf", format);
	PARM51 directive needed		*/
/*	removed mic click option and related code */
/*      timeout error returned from memu-6-4 setting => put reset at top */
/*	timing loop tune in t_b_lpt.a51 loop */
/*	bug fix in months(), get_epday() endless for loop */	
/*	init_io() must be after set_first() as baud_rate is not fixed */
/* Keil C251 log() bug corrected Jan.07 1999 */
/* Timer-2 for CAT port baud rate generation 	Jan.10 1999 */
/* For Kaneko-san  YAESU-wait configurable */
/* icom_wait and yaesu_wait in assembler was unified Jan.13 1999 */
/* ICOM_SEPARATE introduced */
/* T_B_LPTZ.A51  C251 parameter passing => save ROM size  Jan.21 1999 */
/* defaulr kepele for 1999 Jan. 23 1999 */
/* enable i_wait from station.dat */
/* y_wait is fixed 4 m sec in assembler */
/* TRK_CORREC changed to 10Hz.		Mar.05 1999 */
/* uplink freq. correction refference changed 300->100Hz */
/* down link freq. correction refference 100:FM 10:SSB */
/* F_SLEW_VAL[sat_max] introduced for manual +/- correction memory */
/* bug in FT-847 selection from station.dat corrected 99/03/24 */
/* 251cx	MIC click CAT problem		99-04-19 */
/* "Ant. direction"   display 			99-04-19 */
/* Minor problem in Kepele uploading " ",'\0' */
/* bug fix in K0 input from keybourd,   99-04-20 */
/* Start 251350d	Jan.03 2000		*/
/* change in satellite name read_in from element file	Jan.05 2000 */
/* v3.50e	sideral time tabl exteded Y2020		Nov.09 2004 */
/* v3.60a	rename the code number			Sept.22 2013 */
 
/*******************************************************************/

#include <reg251s.h>            /* define 80c251 registors */
...
#define    TRK_FRQ_FM   100    /* C251:Tracking ref freq 200Hz for FM mode */
#define    TRK_CORREC	10    /* manual correction step 100Hz  */
...
#define		ICOM821		0x0001
#define		YAESU736	0x0002
#define		YAESU847	0x0004
...
#define    G0      7.5369793e13		/* GM of Earth in (Orbit/day)^2/Km^3 */
#define    G1      1.0027379093		/* Sidereal/Solar Time rate ratio *
#define    P11     3.1415926535		/* Pi */
#define    P2      6.2831853070		/* Pix2 */
#define    P0      0.017453292	   	/* P1/180 */

#define    R0      6378.160		/* Earth Radius */
#define    F       0.0033528131979	* 1/F F:298.257= earth flattening coefficient */
#define    F2      0.0016764065889	/* F/2 */
...
static UCHAR *greeting[]={
	"\r\n\r\n\r\n\t****************************************\r\n",
	"\t*      TrakBox-II  80C251  v3.60a      *\r\n",
	"\t*   (C) JAMSAT/JA6FTL   Sep.22 2013    *\r\n",
	"\t****************************************\r\n",
	" TrakBox 80C251 \0",  		  /* Title display for LED */
	"  version 3.60a \0",
	"(C)JAMSAT/JA6FTL\0",
	"\t\t\t",
	__DATE__  ,
...
DBL  local_inf[4];	/* 0:local_lat, 1:local_lon, 2:local_lin_c, 3:local_high */

UCHAR	i_wait;

UINT  data adc_data;
short data adc_az,adc_el;

#ifdef NOTDEF
UCHAR   pulse_w,pulse_i;	/* mic click pilse width and interval */
UINT  pll_lock;	   /* ADC read value in modem lock */
UINT  pll_reff;         /* ADC read value one click up */
#endif

UINT   data  tm_year;
UCHAR  data  tm_mon,tm_day,tm_hour,tm_min,tm_sec;
UCHAR  data  time_ch[13];
UINT  rs_buff_mark;	/* RS buffer max point */

/************************/
/*  Array variables     */
/************************/

static UCHAR	baud_tab[6] =		 /* COM port baud rate */
	{0x40,0xD0,0xE8,0xF4,0xFA,0xFD}; /* 300,1200,2400,4800,9600,19200 */

static USN int lpt_baud_tab[5]=
        {0xFD00,0xFF40,0xFF40,0xFFA0,0xFFD0};   
		/* 1200,4800,4800,9600,19200,38400 */
   	
UCHAR baud_rate,lpt_baud_rate;

static UCHAR day_tab[2][13]={
	{0,31,28,31,30,31,30,31,31,30,31,30,31},
	{0,31,29,31,30,31,30,31,31,30,31,30,31}
	};

static DBL G2_tbl[30]={   /* Greenwitch Sideral Time table  added 04-01-20 */
...
UCHAR O_name[SATMAX][20];
UINT  Y02[SATMAX];      /* epoch Year */
DBL   T02[SATMAX];
DBL   M02[SATMAX];
DBL   N02[SATMAX];
DBL   I02[SATMAX];
DBL   E02[SATMAX];
ULONG  K02[SATMAX];	/* Epock # */
DBL   W02[SATMAX];
DBL   O02[SATMAX];
DBL   N12[SATMAX];      /* Decay rate  */
DBL   F01[SATMAX];      /* Downlink Freq */
DBL   F02[SATMAX][4];   /* Uplink Freq */
UCHAR UPLINK_CH[SATMAX];	/* Uplink channel 0-4 */
DBL   F0CONV[SATMAX];	/* F0 correction for converter */
DBL   F1CONV[SATMAX];   /* F1 correction for converter */
DBL   CORRECT_MIC[SATMAX]; /* Doppler correction value for mic click mode */
UCHAR MDU[SATMAX];      /* Uplink mode */
UCHAR MDD[SATMAX];      /* Dwnlink mode */
UINT  SAT_FLAG_SW[SATMAX];  /* Flag SW for each satellite */
long  F_SLEW_VAL[SATMAX];   /* manual slew for +/- correction */

char	sat_list[REG_MAX+1];	/* Control SAT LIST */

UCHAR   local_call[20];             /* Use LOCAL CALLsign */
UCHAR   rs_buf[RS_BSIZE];           /* MSG BUFF */

/*************************/
/* Kepele variables      */
/*************************/

DBL      G2,I0,O0,E0,W0,M0,N0,K0;
DBL      TX0,NX0;
DBL      FREQ1;         /* Base freq for doppler calculation DOWN LINK */
DBL	 FREQ2;	   	/* '''''                               UP LINK */
DBL      T;
DBL	 DA;
UINT     Y2;
...
UINT  az_mini;                 /* ROTATOR MINI VALUE    */
UINT  az_max;                  /*         MAX           */
UINT  el_mini;                 /* ELEVATION MINI        */
UINT  el_max;                  /*           MAX         */
...
UCHAR   rig_maker;	/* Rig variation for CAT command */ 
UCHAR   yaesu_tx_cmd;	/* YAESU freq set command char */
UCHAR   yaesu_rx_cmd;	/* YAESU freq set command char */
UCHAR   i_rx_address;	/* ICOM RX destination address  */
UCHAR   i_tx_address;   /* ICOM TX destination address  */
UCHAR   icom_r;
UCHAR   icom_t;

ULONG   trk_frq_d;      /* temporaly freq for command step */
ULONG   trk_frq_u;
long    trk_frq_correct;	        /* manual track frequency correction by +,- */

float   frq_park[3];	/* 0:RX park freq, 1:TX park freq  */

ULONG   trk_frq_band;	/* 20/200 Hz  */

UCHAR   trx_mode[4];     /* 0:up_mode,1:dwn_mode,2:up_park_mode,3:dwn_park_mode*/
...
DBL C[13];
DBL A,E,E1,S1,C1,O,W,Q;
DBL M9,M,S3,C3,R3,M1,R,R9;
DBL X,X0,X9,X5,Y,Y0,Y9,Y5,Z,Z0,Z1,Z9,Z5;
DBL G7,S7,C7,L8,S9,C9,S8,C8,S5;
DBL R5,Z8,X8,Y8,C5,E9,B5,A9,L5,W5,F9U,F9D;
DBL VX9,VY9;
...
ULONG   DX,mem_DX_az,mem_DX_el; 	/* sec of the year */
UINT    err_timeout,err_recover_tm;

UCHAR   sat_mx;         /* RAM stored max number */
UCHAR   ld_sat;         /* sat number for load to selection menu */
char    obj_sat;      /* Tracking object satellite (0-14) */
UCHAR   m_track_num;     /* satellites number of multi tracking mode */

#ifdef NOTDEF
ULONG   fd0_los;	/* Freq. at LOS for step control */
ULONG   fdx_los;	/* Freq. for refference */
ULONG   step_size;	/* step freq value */
#endif

UCHAR   zzzcall[10];	/* RAM backup fail check ID */
...
void   set_date(UCHAR *) small;
void   read_adc(UCHAR) small;
...
void   spout(USN char);                 /* Serial printer out */
void   spout2(USN char);		    /* Serial printer out-2 */
...
void   print_lcd(USN char *) small;
void   months(void) small;
void   send(UCHAR *) small;
UINT   day_of_year(UINT,UCHAR,UCHAR);
void   sel_sat(char);
void   set_def(void);
...
void   get_epday(UINT,UCHAR,UCHAR,UCHAR,UCHAR,UCHAR);

void   month_day(UINT,UINT *,UCHAR *,UCHAR *);
void   to_hour(UCHAR *,UCHAR *,UCHAR *,DBL);

void   set_first(void) small;
void   set_stn_inf(void);		/* set station data */

char   disp_yn(char *) small;
DBL    get_fval(char *,short *);
short  get_val(char *,short *);
/*
long   get_lval(char *,short *);
*/
void   sys_config(void);
void   calib_ant(void);
void   value_ant(void);

UCHAR  update_menu(void);
void   up_station(void);
UCHAR  config_menu(void);
...
void   wait(UINT);
void   reset_timeout();
void   icom_cat_header(UCHAR);
void   go_parking(char);
...
void   init_io() small
{
...
#ifdef NOTDEF
   TH2 = RCAP2H = 0xfe;    /*   8052 Timer-2 baud rate =4800    */         
   TL2 = RCAP2L = 0x80;    /*   65536 - Fx/12*4800 = 0xFE80  Fx:22118400 */
                           /*                *9600 = 0xFF40
                           /*                *19200 = 0xFFA0 */
                           /*                *1200 = 0xFA00 */
                           /*                *2400 = 0xFD00 */
   ...
   TH2 = RCAP2H = (char)((lpt_baud_tab[lpt_baud_rate] & 0xFF00) >> 8);
   TL2 = RCAP2L = (char)(lpt_baud_tab[lpt_baud_rate] & 0x00FF);
...
void   lcd_cmd(USN char ch)	/* new for 80c251 */
{
	ECI = 0;	/* RS */
	T2EX = 0;	/* R_W */
	XBYTE[PPI] = ch;
...
{
       send("\x07\r\n\tData in RAM was lost !! Set default parameter !\r\n");
	   ...
	   trk_frq_band = (long)TRK_FRQ; /*  10Hz default */
	   ...
	   i_wait = 45; /* 7mse per inter command */
...
UCHAR  data  i;
	   for(i=0;i<SATMAX;i++){
		 ...
		 F_SLEW_VAL[i] = CORRECT_MIC[i] = F0CONV[i]= F1CONV[i]= 0;
		 K02[i]= T02[i]= 1;		 
		 MDU[i] = MDD[i] = '\0';
...
UCHAR read_mode() small        /* Binary SW read & ADC READ_MODE */
{
...
void   sendp(char *s) small                 /* out to LP port */
...
void   sendp2(char *s)                   /* out to Alternate LP port */
{
UCHAR data ch;
...
short  get_val(char *msg,short *size)
...
/*
long	get_lval(char *msg,short *size)
{
UCHAR	temp[80];
	send(msg);
	getline(temp);
...
DBL  get_fval(char *msg,short *size)
{
UCHAR    temp[80];
...
void   set_date(UCHAR *s) small             /* RTC SET */
{
UCHAR data  i,j,k;
...
void   read_adc(UCHAR ch) small           /* Read ADC & store (adc_data) */
{
UINT   data   adata,data_h,data_l;
UCHAR  data   port;

       port = (UCHAR)(ch & 0x07);    /* MAX CH = 8 */
       XBYTE[NEC_ADC] = port;           /* select channel */

       wait(30);

       data_h = (UINT) (XBYTE[NEC_ADC+1]& 0xff);
...
short	temp;
	...
	send("\r\n\t==== Set CAT wait time ====\r\n\r\n");
	printf("\tPresent value:yaesu_wait = %d, icom_wait = %d\r\n",y_wait,i_wait);
	
	if(temp=get_val("\tYAESU wait timing value ? [1-255] ",&cs))
		y_wait =(UINT)temp;
	if(temp=get_val("\tICOM  wait timing value ? [1-255] ",&cs))
		i_wait =(UINT)temp;
}
#endif

UCHAR  sel_main_menu()		/* Main menu display */
{
UCHAR	data    ch;
...
UCHAR    *msg[] ={
	         "South",
	         "North"};
UCHAR    *BAUDmsg[] ={		/* COM port Baud rate display */
	 	 " 300",
		 "1200",
		 "2400",
		 "4800",
...
UCHAR   *Smsg[] ={                       /* Station Elements msg */
               "         Callsign",
               "         Latitude",
               "        Longitude",
               "   Antenna height",
	...
	if(ch == '2') set_stn_inf();
	   ...
	   baudval = get_val("\t",&cs);
	    if     (baudval == 300 ) baud_rate = 0;
	    else if(baudval == 1200) baud_rate = 1;
	    else if(baudval == 2400) baud_rate = 2;
	    else if(baudval == 4800) baud_rate = 3;
	    else if(baudval == 9600) baud_rate = 4;   
	    else if(baudval == 19200) baud_rate = 5;   
	}	   
}
...
UCHAR mox,dx,hx,mix,sx;
	   ...
	   send("\r\n\tEnter 'Ret.' for Real Time Clock set\r\n\r\n");
        ...
        for(;;){	   
	        months();
                ...
                mix = tm_min;
                sx = tm_sec;
		get_epday(yx,mox,dx,hx,mix,sx);
	   	send("\tPresent Time:UTC ");
           	printf("%4d/%02bu/%02bu %02bu:%02bu:%02bu T:%5.8f\r",tm_year,
                (UCHAR)tm_mon,(UCHAR)tm_day,(UCHAR)tm_hour,(UCHAR)tm_min,(UCHAR)tm_sec,T);
	        if(RI){
			ch = (getchar() | 0x20);
			if(ch == 'y'){
				XBYTE[RTC_CD] |= 0x08;
	   ...
	   months();
           send("\r\n\r\n\tSet realtime Clock.\r\n\tPresent Time:UTC ");
           printf("%4d/%02bu/%02bu %02bu:%02bu:%02bu\r\n",tm_year,
               (UCHAR)tm_mon,(UCHAR)tm_day,(UCHAR)tm_hour,(UCHAR)tm_min,(UCHAR)tm_sec);
...
void   edit_sat_data(short s_num)     /* DIPLAY SAT DATA & EDIT */
{

       short   cs,ys;
UCHAR    s_txt[100],tmp[20];
       DBL   newf;
float 	 new_val;
UCHAR    *DPLmsg[] = {
			"CAT ",
...
UCHAR   *Mdmsg[] = {	/*  TX/RX mode  msg */
		...
		strcpy(O_name[s_num],tmp);
           ...
           printf("\t%s %6.8f",Dmsg[1],T02[s_num]);
           newf = get_fval("\t\tT0:",&cs);
           if(cs != 0) T02[s_num]=  newf;

           printf("\t%s %6ld",Dmsg[2],K02[s_num]);
           new_val = get_fval("\t\tK0:",&cs);
           if(cs != 0) K02[s_num]= (ULONG)new_val;

           printf("\t%s %6.4f",Dmsg[3],M02[s_num]);
           newf = get_fval("\t\tM0:",&cs);
           if(cs != 0) M02[s_num]=  newf;

           printf("\t%s %6.8f",Dmsg[4],N02[s_num]);
           newf = get_fval("\t\tN0:",&cs);
           if(cs != 0) N02[s_num]=  newf;

           printf("\t%s %6.4f",Dmsg[5],I02[s_num]);
           newf = get_fval("\t\tI0:",&cs);
           if(cs != 0) I02[s_num]=  newf;

           printf("\t%s %6.7f",Dmsg[6],E02[s_num]);
           newf = get_fval("\t\tE0:",&cs);
           if(cs != 0) E02[s_num]=  newf;

           printf("\t%s %6.4f",Dmsg[7],W02[s_num]);
           newf = get_fval("\t\tW0:",&cs);
           if(cs != 0) W02[s_num]=  newf;

           printf("\t%s %6.4f",Dmsg[8],O02[s_num]);
           newf = get_fval("\t\tO0:",&cs);
...
**/     
           if(getline(s_txt))
		   sscanf(s_txt,"%lf %lf %lf %lf",&F02[s_num][0],&F02[s_num][1],
...
UCHAR    ch;
           ...
           send("\t5. Monitor ADC value and antenna position\r\n"); 
		   ...
		   case	   ESC:
		       ...
		       radio_p_conf();
                   ...
                   case    8:		/* Mic click configuration */
               	       send("\r\n\tThis function was eliminated in C251 version");
			/*  mic_step_con(); */
                       break;
	...
	switch(getchar() | 0x20){
		...
		case 'y':
					...
					SW3 &= ~YAESU847;
                        ...
                        send("\r\n\tSelect FMN mode ? [Y/N] ");
                        if((getchar() | 0x20 ) == 'y') SW |= YAESU_FMN;
                        else SW &= ~YAESU_FMN;
/*
                        printf("\r\n\tWait time for character [%d] ",y_wait);
                        if(temp=get_val(" [1-255] ",&cs)) y_wait =(UINT)temp;
*/
			break;
		case 'i':
			rig_maker = ICOM;
                        printf("\r\n\tEnter destination Address for RX (%BX) ",
                                        			i_rx_address);
                        if(getline(tmp)) sscanf(tmp,"%bx",&i_rx_address);
                        printf("\tEnter destination Address for TX (%BX) ",
                                        			i_tx_address);
                        if(getline(tmp)) sscanf(tmp,"%bx",&i_tx_address);

			printf("\tSeparated TRX ? [Y/N]  ");
                        if((getchar() | 0x20 ) == 'y') SW3 |= ICOM_SEPARATE;
                        else SW3 &= ~ICOM_SEPARATE;

                        printf("\r\n\tWait time for command [%bd] ",i_wait);
                        if(temp=(UCHAR)get_val(" [1-255] ",&cs)) i_wait =temp;

			break;
	}
			
	lpt_baudval = get_val("\r\n\tPort baud rate ? [1200/4800/9600/19200] ",&cs);
	if(     lpt_baudval == 9600)  lpt_baud_rate = 3;
	else if(lpt_baudval == 19200) lpt_baud_rate = 4;
	else if(lpt_baudval == 4800)  lpt_baud_rate = 2;
	else if(lpt_baudval == 1200)  lpt_baud_rate = 0;
...
UCHAR  data   i;
           ...
           printf("\r\n\r\n\r\n\t ========== Realtime Tracking [%4d/%02bu/%02bu] =========\r\n",
		tm_year,(UCHAR)tm_mon,(UCHAR)tm_day);
	...
	for(i=0;i<SEL_MAX;i++) ctr_az[i] = ctr_el[i] = '\0';
  	...
  	SW2 |= PARK_TRX_FLAG;
        ...
        if(SW & RUN_MODE){     /* if standalone mode */
                ...
                if((sel1 < 0) || (sel1 >= (char)m_track_num))	sel1 = 0;
	}
        else{		       /* terminal mode */
		disp_list_sat();
                if(obj_sat == -1) return;
		if((sat_list[obj_sat] < 0) || (obj_sat >= (char)m_track_num)){
	...
	for(;;){  
		...
		if(sel == lcd_act_sat){
	        ...
	        if((P1 & P14) == 0) disp_ant();
				...
				mark,(UCHAR)tm_hour,(UCHAR)tm_min,(UCHAR)tm_sec);/* UTC */
	        	printf("%5.1f/%-6.1f%6.1f/%-6.1f",ctr_az[0],ctr_el[0],ctr_az[1],ctr_el[1]);
	        	printf("%6.1f/%-6.1f%6.1f/%-6.1f",ctr_az[2],ctr_el[2],ctr_az[3],ctr_el[3]);
		        printf("%6.1f/%-6.1f\r",ctr_az[4],ctr_el[4]);

			lcd_cmd(0x80);	       /* set DD RAM to line-1 top */

	               	sprintf(lcd_text,"%02bd:%02bd:%02bd%c%-7s",
				(UCHAR)tm_hour,(UCHAR)tm_min,(UCHAR)tm_sec,mark,O_name[lcd_act_sat]);
        		print_lcd(lcd_text);
		...
		if((E9 > p16_on_el) && (SW & TRACK_ID)){ 
			...
			if(SW & BREAK_M) return;
		 	...
		 	XBYTE[LED_PORT] &= ~AOS_LED; /*AOS-LED OFF*/
			if((SW & ANT_PARK) && (SW & TRACK_ID)) go_parking(sel);
			else{
				XBYTE[PPI+2] &= 0xf0; /* AZ/EL STOP */
				reset_timeout();
			}
...
UCHAR   ch,lcd_text[30], mark,no_mark = NON_ST,mark_ = TRACK_ST;
...
newsat_found:
   	   ...
   	   SW2 |= (MODE_CMD_FLAG | PARK_TRX_FLAG);
	   ...
	   trk_frq_correct = F_SLEW_VAL[obj_sat]; /* new in c251 */

	   if(rig_maker == YAESU){  /* YAESU   */
			yaesu_cmd(0x00,0x00);
			if(SW3 & YAESU736){
				yspout(0xC9);yspout(0x60);yspout(0x00);   /* 92-10-17 */
        	        	yspout(0x00);yspout(yaesu_rx_cmd); /* dummy 1296.0MHz */
				yspout(0x05);yspout(0x10);yspout(0x00);   /* 92-10-17 */
                		yspout(0x00);yspout(yaesu_rx_cmd); /* dummy 51.0000MHz */
                	}
			else if(SW3 & YAESU847) yaesu_cmd(0x00,0x4E);
			yaesu_cmd(0x00,0x80);
	   }
	   ...
	   if(FREQ2 > 0.1) rig_set_u(act_sat);   
           ...
           trk_frq_band = (long)TRK_FRQ;      /* default 10Hz step */
	   if(trx_mode[1] == FM) trk_frq_band = (long)TRK_FRQ_FM; /* 100Hz step  */
...
/*******************************************************/
		   ...
		   if((SW & MULTI_MODE) == 0){  /* if not multi mode */
			...
			case ' ':
				obj_sat++;
				if(obj_sat >= (char)ld_sat) obj_sat--;
				break;
			...
			case '+': /* Doppler compensation + */
		       		F_SLEW_VAL[obj_sat] = trk_frq_correct += TRK_CORREC;
				break;
	     ...
	     if((P1 & P14) == 0) disp_ant();
		     ...
		     printf("%c %02bd:%02bd:%02bd",
		                     mark,(UCHAR)tm_hour,(UCHAR)tm_min,(UCHAR)tm_sec);/* UTC */
	     	     printf("%8.2f%8.2f%9.1f%9.1f",A9,E9,R5,R-R0); 
	     	     printf("%7.1f%8.1f%7.1f%+6.0f",L5,W5,M9,F9D); 
		     printf("/%-5.0f \r",(DBL)trk_frq_correct); 

	   	     lcd_cmd(0x80);	       /* set DD RAM to line-1 top */
        	     sprintf(lcd_text,"%02bd:%02bd:%02bd",
		     	(UCHAR)tm_hour,(UCHAR)tm_min,(UCHAR)tm_sec);
	    	     print_lcd(lcd_text);

        	     sprintf(lcd_text,"%c%-5s  ",mark,O_name[sat_list[obj_sat]]);
	    	     print_lcd(lcd_text);
...
*/
             if(SW & MIC_CLICK);
                   ...
                   if((labs(trk_frq_u - RFU) > 100l) && (SW & DOPPL_UPL)){ 
...
UINT 	tm,ts;
		...
		if((tm_sec == ts) && (tm_min - tm) == err_recover_tm){
...
#ifdef NOTDEF
void  f_step_con(char act_sat)     /*  Doppler RX freq up down control */
{       		        /*  92-02-20 */
	short   now_pll;
	short	pll_range = (pll_reff - pll_lock);
UINT    j,pulse_width = pulse_w * 1000;
...
void step_ini()             /* Auto step pulse generate at AOS */
{                               /* 92-12-03 */
	UINT	i,j,pulse_width,pulse_int;
             long       f_diff;
	ULONG  long       pulse_n;
...
void  yaesu_cmd(UCHAR D1,UCHAR P1)
...
void icom_cat_header(UCHAR ch)
	/* destination address */
{
                spout(0xfe);spout(0xfe);
                spout(ch);spout(0x02);
}

void   rig_set_d(char sel)          /*  Downlink Doppler correction by CAT  */

{
   ...
   UCHAR     s_txt[40];
		...
		chl = chh = 0x00;
                ...
                yspout(yaesu_rx_cmd);                   /* FULLDUP RX  Freq  */
                yaesu_cmd(0x00,0x80);
        }
		...
		icom_cat_header(icom_r);
        	spout(0x07);spout(0xD1);       /* Sub band access  */
                spout(0xfd);
		icom_wait(i_wait);
	    }
                icom_cat_header(icom_r);
                spout(0x00);    /*  freq. set command    */
                ...
                spout(0xfd);    /*   CI-V tail char     */
		icom_wait(i_wait);
        }
}

void   rig_set_u(char sel)          /*  Doppler correction for uplink by CAT  */

{                           /*  Mar. 28 92  */
   char         i;
   UCHAR     ch,chl,chh,s_txt[40];
   ULONG     fl;
		...
		chl = chh = 0x00;
                ...
                yspout(yaesu_tx_cmd);                   /* FULLDUP RX  Freq  */
                yaesu_cmd(0x00,0x80);
        }
		...
		if(!(SW3 & ICOM_SEPARATE)){
                        ...
                        spout(0x07);spout(0xD0);  /* Access main band  */
                        spout(0xfd);
			icom_wait(i_wait);
		}
                        icom_cat_header(icom_t);
                        spout(0x00);              /* Set freq command */
                        for(i=9;i>0;){
                                chl = chh = '\0';
...
void rig_mode_set(UCHAR sel)     /* TRX mode set for up/dwn  */

{
        UCHAR    D1,s_txt2[40],s_txt3[40];
		...
		if(!(SW3 & ICOM_SEPARATE)){
                        icom_cat_header(icom_t);
                        spout(0x07);spout(0xD0); /* main band access */
                        spout(0xfd);
			icom_wait(i_wait);
		}
                        icom_cat_header(icom_t);
                        spout(0x01);   /* Set mode command */
			switch(trx_mode[0]){
				case LSB: spout(0x00);break;
				case USB: spout(0x01);break;
				case CW:  spout(0x03);break;
				case FM : 
				default : spout(0x05);
			}
                        spout(0xfd);
...
void   months() small	/* get time and calculate T */
{
UINT	n,i;
...
UINT  day_of_year(UINT year,UCHAR month,UCHAR day)
...
void   sel_sat(char sel)      /* read object elements for selected sat. */
...
int	g2_sel;
      ...
      g2_sel = Y2 - 1991;
      if((g2_sel<0) || (g2_sel>30)) g2_sel = 29;      /* 04-11-09 */
...
*/
      ...
      RFU += (long) (F1CONV[sel]*1000000l);
       ...
       S1 = sin(I0*P0);		/* 10210 */
       C1 = cos(I0*P0);		/* 10210 I0=inclination */
				/* get observer's geocentric coordinate */
       L8 = local_inf[0]*P0;	/* 10550 */
       S9 = sin(L8);
       C9 = cos(L8);
       S8 = sin(-1.0*local_inf[2]*P0);	/* 10560 */
       C8 = cos(local_inf[2]*P0);
					/* flattened orbital spheroid earth */
       R9 = R0*(1-F2+F2*cos(2*L8))+local_inf[3]/1000.0;	/* 10580 */
       L8 = atan(0.993305458*S9/C9);	/* 10600 */
       Z9 = R9 * sin(L8);		/* 10600 */
       X9 = R9 * cos(L8)*C8;		/* 10610 */
       Y9 = R9 * cos(L8)*S8;		/* 10610 */

       VX9 = -R9*cos(L8)*S8*0.0000729211585; /*  10609 JR1HAL  */
...
void   main_loop(char sat_sel)

{
       DBL  N,V0,VX,VY,VZ,X1,Y1,VX1,VY1,VZ1;
       DBL  M5,C0,S0,E2,K2,C2,S2;
       DBL  K,R8,TXG,Q,Q0,VX0,VY0;
       long   F9DT1,F9DT2,F9DT3;

       TXG = T-TX0;    /* step */

       N = N0 + 2*TXG*NX0;       	  /* 10154 Mean Motion at T */
				/* NX0=N12= decay rate, N0=mean motion */

       A =  pow((G0/(N*N)) ,0.333333333); /* 10156 SMA at Epoch T */

       V0 = A*N*0.00007272205217;	  /* 10161 Mean velocity of satellite jr1hal */

       E2 = 1.0- E0*E0;			/* 10160 Q0 = initial orbit phase */
       E1 = sqrt(E2);
       Q0 = M0/360.0 +K0;		/* 10160 */

       K2 = 9.95*pow(R0/A,3.5)/(E2*E2);      /* 10190 flattened oblate sphe. */

       O = O0-TXG*K2*C1;		/* 10220 */
       S0 = sin(O*P0);			/* 10230 */
       C0 = cos(O*P0);			/* 10230 O:RAAN */
                                              
       W =W0+TXG*K2*(2.5*(C1*C1)-0.5);	/* 10240 */
       S2 = sin(W*P0);    		/* 10250 W=argument of Perigee */
       C2 = cos(W*P0);

	/* Setup coordination matrix for the current orbit */
       C[6]  = (C2*C0)-(S2*S0*C1);		/* 10270 */
       C[10] = -1.0 *(S2*C0)-(C2*S0*C1);	/* 10270 */
       C[7]  = (C2*S0)+(S2*C0*C1);		/* 10280 */
       C[11] = -1.0 *(S2*S0)+(C2*C0*C1);	/* 10280 */
       C[8]  = (S2*S1);				/* 10290 */
       C[12] = (C2*S1);				/* 10290 */

	/* Start evaluate M(Mean anomaly) in (o-2*PI) range */

       Q = Q0 + N0*TXG + NX0*(pow(TXG,2.0));    /* 10345 rev. to include drug */     
       ...
       M  = (Q-K)*P2;	
       			/* 10346 */
/*       K = K + K0; */		/* 10350 present orbit # */
/*
       Q = Q + K0;	        no meaning at the moment 
*/
	/**** routine to solve KEPLER's equation given M & return the
			satellite's GEOCENTRIC coordinates ******/

       E = M + E0 * sin(M) + 0.5*(E0*E0)*sin(2*M); /* 10400 initial trial value */

       for(;;){	 	/* 10410 Solve trancendtal Kepler's equation  */
           S3 = sin(E);
           C3 = cos(E);
           R3 = 1.0 - E0*C3;
           M1 = E- E0*S3;
           ...
           if(fabs(M5) < 1e-6) break;	/* 10430 */
           E = E - M5/R3;
       }

	/****** get satellite's XYZ coordinates *******/
       X0 = A *(C3-E0);	        /* 10450 In the plane of the orbit */
       Y0 = A*E1*S3;
       R  = A*R3;
       
	VX0 = -V0*S3/R3;	/* 10450  JR1HAL  */
	VY0 =  V0*E1*C3/R3;     /* Velocity on XY plane (Km/sec) */          

	/****** Rotate frm orbit plane to INERTIA CLESTIAL coordinates *****/
       X1 = X0 * C[6]+Y0*C[10]; /* 10470 */
       Y1 = X0 * C[7]+Y0*C[11];
       Z1 = X0 * C[8]+Y0*C[12]; 
       
       VX1 = VX0*C[6] + VY0*C[10];	/* 10471 JR1HAL */
       VY1 = VX0*C[7] + VY0*C[11];      /* Velocity vector */
       VZ1 = VX0*C[8] + VY0*C[12];		
       
       /**** Rotate thru current GHA of ARIES, convert to GEOCENTRIC coordinates ***/
       G7 = T*G1+G2;                  /* 10490 */
       G7 = (G7 - floor(G7))*P2;
       S7 =  -1.0*sin(G7);
       C7 = cos(G7);
	...
	/***** Rotate into OBSERVER's LOCAL coordinates ******/
       Z8 = (X5*C8*C9) + (Y5*S8*C9)+(Z5*S9);	/* 10720 */
       X8 = -1.0*(X5*C8*S9)-(Y5*S8*S9)+(Z5*C9); /* 10730 */
       Y8 = (Y5*C8)-(X5*S8);			/* 10730 */
       S5 = Z8/R5;				/* 10740 */
       C5 = sqrt(1.0 - S5*S5);
       E9 = (atan(S5/C5))/P0;  		/* 10740 E9 = Elevation */
       					/* A9 = Azimuth */
       A9 = (atan(Y8/X8))/P0;		/* P0=0.017453292,1/P0=57.296083 */
       B5 = Z/R;			/* 10770 */
       L5 = (atan(B5/(sqrt(1.0-B5*B5))))*57.29608; /* 10770 SSP Latitude */
       W5 = (atan(Y/X))*57.29608;
	...
	R8 = ((VX-VX9)*X5 + (VY-VY9)*Y5 + VZ*Z5)/R5;	/* 10680 JR1HAL */
...
UCHAR    i;       
      ...
      Y02[0] = 13;
      T02[0] = 254.74297939;     /* Epoch Time */
      K02[0] = 84301;           /* Epoch rev. */
      M02[0] = 170.2427;        /* Mean Anomaly */
      N02[0] = 13.53005152;     /* Mean Motion */
      I02[0] = 98.5457;         /* Inclination */
      E02[0] = 0.0349499;       /* Eccentricity */
      W02[0] = 299.9056;        /* Arg. of Perigee */
      O02[0] = 301.6933;        /* R.A.A.N. */
      N12[0] = 1.0e-07;        /* Decay rate */
      F01[0] = 435.910;
      F02[0][0] = 145.870;
      F02[0][1] = 145.850;
      F02[0][2] = 145.890;
...
UINT	i;
   ...
   for(;;){
	...
	printf("\r\n\r\n\t1:%5.1f/%-4.1f 2:%5.1f/%-4.1f 3:",
		PRESET_AZ[0],PRESET_EL[0],PRESET_AZ[1],PRESET_EL[1]);
	printf("\r\n\r\n\t1:%5.1f/%-4.1f 4:%5.1f/%-4.1f",
		PRESET_AZ[2],PRESET_EL[2],PRESET_AZ[3],PRESET_EL[3]);
...
void   rotor_con1(short sel)     /* PRESESSION for ROTATOR CONTROL */
	       ...
	       ch = getchar() | 0x20;
			...
			printf("\tRX parking mode (F/U/L) : ");
	   		if(getline(tmp)){
				...
				case 'n': trx_mode[3] = NONE;
	   		    }
			}
			printf("\tTX parking mode (F/U/L) : ");
	   		if(getline(tmp)){
			    switch(tmp[0] | 0x20){
...
UINT  temp = SW;
       ...
       switch(rig_maker){
		...
		case YAESU:
			...
			break;
		case ICOM :
			printf("  (%BX)",i_rx_address);
			printf("(%BX)",i_tx_address);
			printf("(w=%bd)",i_wait);
			break;
       }
...
void calc_sat_sched(short num)

{
static UINT  y_dsp,day_of_year,yx;
static UCHAR  m_dsp,d_dsp,h_dsp,mi_dsp,s_dsp;
UCHAR  mox,dx,hx,mix,sx;
UCHAR  i,ch,temp[40], flg = NO, scr_flg = NO;
      short   cs;
/* UINT  m_dsp,d_dsp,h_dsp,mi_dsp,s_dsp; */
DBL   minx = 1.0;
DBL   secx = 0.0;
DBL   tsp,fx1,n_minx,n_secx,tmp_sat_el;

           sel_sat(num);	/* read the elements. */
           months();
           printf("\r\n%4d/%02bu/%02bu %02bu:%02bu:%02bu\r\n",
	   	tm_year,(UCHAR)tm_mon,(UCHAR)tm_day,(UCHAR)tm_hour,
		(UCHAR)tm_min,(UCHAR)tm_sec);
	   ...
	   n_secx = get_fval("\tSec [0]: ",&cs);
           if(cs != 0) secx = n_secx;
	   minx *= 0.0006944444444;              /*Day / sec */
           secx *= 0.00001157407407;
           ...
           for(;;){
		       ...
		       scr_flg = NO;
                   ...
                   month_day(day_of_year,&y_dsp,&m_dsp,&d_dsp);
                   to_hour(&h_dsp,&mi_dsp,&s_dsp,fx1);

                   if(E9 > -1.0){

                       printf("%4d/%02bd/%02bd ",y_dsp,(UCHAR)m_dsp,(UCHAR)d_dsp);

                       printf(" %02bd:%02bd:%02bd    ",(UCHAR)h_dsp,(UCHAR)mi_dsp,(UCHAR)s_dsp);

                       printf("%5.1f/%4.1f%9.1f%+9.0f\r\n",A9,E9,M9,F9D);
...
void   get_epday(UINT yx,UCHAR mox,UCHAR dx,UCHAR hx,UCHAR mix,UCHAR sx)
{
UINT    n,i;
DBL     th,tm,ts;
       ...
       if(n = (yx - Y2)){
                for(i=1;i<=n;i++){
                	...
                	if(i > 5) break;	/* c251 debug */
		}
       }       

       T = (DBL)DA*1.0;             /* to float */
       T += (th + tm + ts);
}

void month_day(UINT day_of_year,UINT *year_x,UCHAR *pmonth,UCHAR *pday)
{
UINT year,yd;
char      leep=1,i;
...
void   to_hour(UCHAR *th,UCHAR *tm,UCHAR *ts,DBL tos)
{
       DBL fx1;
       short   hour_dsp,min_dsp,sec_dsp;

           fx1 = tos * 24.0;
           hour_dsp =(short) fx1;
           fx1 =fx1 - (DBL)hour_dsp;
           min_dsp = (short)( fx1*60.0);
           fx1 = fx1 -(DBL)(min_dsp/60.0);
           sec_dsp = (short)(fx1*3600.0);
...
UCHAR    k,ch,line[80],tmp[80];
       ...
       clr_buff();	/* clear the whole SAT data */
       send("\r\n");

                for(i=0;;){
                        for(k=0;k<79;){		/* read one line */
                           ch = rs_buf[i++];
                           if(ch == EOFF) break;
                           if(ch == CR || ch == LF) break;
                           tmp[k++] = ch;
			...
			if(line[2] == 'A') continue;	/* ignor garvage Jan.05 2000 */
...
G-ECCENTRICITY H-ARGPERIGEE I-MNANOM J-MNMOTION K-ORBITNUM Z-CHECKSUM
AO-10
1 14129U 83058B   95049.74423524 -.00000256  00000-0  10000-3 0  3407
2 14129  26.5642 282.2218 6017196 253.4768  35.4795  2.05881639 87868
UO-11
1 14781U 84021B   95051.05481353  .00000200  00000-0  41702-4 0  7768
2 14781  97.7809  59.1025 0011198 191.6804 168.4140 14.69306688586705
***********/
...
{		          /* 92-04-14 */
UINT    i;
UCHAR   j,k,m=0,n,o,p,q,r,ch,tmp[100],tmp1[20],tmp2[40],tmp21[20],
...
{
UINT  i,baudval = 0,lpt_baudval = 0;
UCHAR   k,ch,tmp1[80],tmp5[20],tmp6[20],tmp7[20];       
...
void host_mode()	/* Host mode 09-28-92 */
{
 ...
 UCHAR	size,k,ch,tmp[30],temp[80],temp1[30],temp2[30];
